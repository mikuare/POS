...
function markSupabaseNetworkBackoff(error, context = '') {
  if (!isLikelySupabaseNetworkError(error)) return false;

  const now = Date.now();
  supabaseNetworkBackoffUntil = Math.max(supabaseNetworkBackoffUntil, now + SUPABASE_NETWORK_COOLDOWN_MS);
  const backoffSeconds = Math.ceil((supabaseNetworkBackoffUntil - now) / 1000);

  if ((now - lastSupabaseBackoffLogAt) > 5000) {
    const where = context ? ` during ${context}` : '';
    console.warn(`[Offline] Supabase network unreachable${where}; pausing remote attempts for ${backoffSeconds}s.`);
    lastSupabaseBackoffLogAt = now;
  }

  return true;
}

function isSupabaseInNetworkBackoff() {
  return isSupabaseEnabled() && Date.now() < supabaseNetworkBackoffUntil;
}
...
function listProductsFallback() {
  ...
  return DEFAULT_PRODUCTS.map((p) => {
    const categoryKey = String(p.category || '').toLowerCase();
    const currentOrder = nextOrderByCategory.get(categoryKey) || 10;
    nextOrderByCategory.set(categoryKey, currentOrder + 10);
    return {
      ...p,
      sortOrder: Number.isFinite(Number(p.sortOrder)) ? Number(p.sortOrder) : currentOrder
    };
  });
}
...
async function listMenuCategories() {
  if (!isSupabaseEnabled() || isSupabaseInNetworkBackoff()) {
  ...
  try {
    ...
    if (!markSupabaseNetworkBackoff(error, 'listMenuCategories')) {
...
async function listProducts() {
  ...
  try {
    ...
    if (!markSupabaseNetworkBackoff(error, 'listProducts')) {
...
async function createMenuProduct({ name, price, category, image, sortOrder }) {
  ...
  if (isSupabaseEnabled()) {
    ...
    return {
      ...
      image: data.image_url || imageUrl,
      sortOrder: Number(data.sort_order || safeSortOrder)
    };
  }
...
async function persistInvoice(invoice) {
  ...
  try {
    ...
    if (!markSupabaseNetworkBackoff(error, 'persistInvoice')) {
...
async function setInvoicePaid(invoiceId, paymentData) {
  ...
  if (isSupabaseEnabled()) {
    ...
    try {
      ...
      if (!markSupabaseNetworkBackoff(error, 'setInvoicePaid')) {
...
async function getSalesReport({ dateFrom, dateTo }) {
  ...
  if (isSupabaseEnabled() && !isSupabaseInNetworkBackoff()) {
    try {
      ...
      if (!markSupabaseNetworkBackoff(error, 'getSalesReport')) {
...
async function getTopSalesPerProduct(limit = 10) {
  ...
  if (isSupabaseEnabled() && !isSupabaseInNetworkBackoff()) {
    try {
      ...
      if (!markSupabaseNetworkBackoff(error, 'getTopSalesPerProduct')) {
...
async function listAllInvoices({ dateFrom, dateTo, status } = {}) {
  ...
  if (isSupabaseEnabled() && !isSupabaseInNetworkBackoff()) {
    try {
      ...
      if (!markSupabaseNetworkBackoff(error, 'listAllInvoices')) {
...
async function syncOfflineQueue() {
  ...
  if (isSupabaseInNetworkBackoff()) return { synced: 0, failed: 0, remaining: offlineQueue.getCount() };
  ...
  for (const op of ops) {
    try {
      ...
      const networkFailure = markSupabaseNetworkBackoff(error, 'syncOfflineQueue');
      if (!networkFailure) {
